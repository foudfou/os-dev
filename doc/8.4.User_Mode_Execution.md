...
This happens immediately! That is right after `INT`, thus before the handler is
executed. Otherwise the handler could not execute in kernel mode. `isr_wrapper`
thus saves the context to the stack pointed by esp0: the process kernel stack.
